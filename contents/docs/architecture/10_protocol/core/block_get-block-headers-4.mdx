...
The `getBlockHeaders4` message is a specialized request designed for light clients. It allows a client to download only the transactions that are relevant to its own wallet addresses, significantly reducing bandwidth and storage requirements.
