...
| `wallet` | `filepath` | Specifies the path to the wallet file (default: `ixian.wal`). |
| `walletPassword`| `password` | Provides the wallet password. |
...
| `blocksDbCache` | `ulong` | The maximum RAM in bytes to allocate for the RocksDB Blocks cache. |
| `activityDbCache` | `ulong` | The maximum RAM in bytes to allocate for the RocksDB Activity cache. |
