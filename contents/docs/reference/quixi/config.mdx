...
The configuration file uses a simple `key = value` format. Each option must be on its own line. Lines starting with a semicolon (`;`) or hash (`#`) are treated as comments and are ignored.
