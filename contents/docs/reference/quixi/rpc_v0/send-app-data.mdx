...
*   **`appId` vs. `protocolId`:** This is the most important concept for this endpoint.
    *   **`appId`:** Use this to broadcast data to a specific app. Currently the node hashes this string to create a `session_id`.
    *   **`protocolId`:** Use this for general, non-app specific communication. The node hashes this string to create a `protocol_id`. For example, `protocolId: "tic-tac-toe-protocol"` could be used to broadcast to any app supporting the "tic-tac-toe-protocol", instead of locking it down to a specific app.
...
*   **Dual End-to-End Encryption:** The data payload is fully end-to-end encrypted before transmission over the S2 network.
